# How you can integrate this all with Discord:

---
INSTRUCTIONS:


# If you dont want to use a template - you can create your own server very easily:

## Step 1 - Create your own Discord Server (or use the template below)

![Example - Create Server](https://i.imgur.com/JrhiM8d.gif)


## Step 2- Turn on Developer Mode in User Settings > Advanced

![Example - Developer Mode](https://i.imgur.com/JQj1mWh.gif)


## Step 3 - Copy your guild ID

![Example - Copy Server ID](https://i.imgur.com/70uhzDz.gif)

## Step 4 - Get your Discord Authorization Token

You'll need this to create your channels programatically.

Press CTRL + SHIFT + I on your windows PC to open the developer window.
---
Click off of the element tab and click the network tab.
---
You'll see a filter box. Just below that - click FETCH/XHR.
---
Start typing into your server's message box.
---
Click the url that pops up - "messages" - click the headers tab,
and scroll down until you see your Authorization Token. 
---

![Token Retrieval Example](https://i.imgur.com/1EU2N5x.gif)



### IF YOU'D RATHER USE THE TEMPLATE: 

Visit this URL:

https://discord.com/template/Qr6TCGR7MjHR

This is the server template that takes a lot of the work out of creating channels for specific coins / 
conditionals. I'll be building upon this as time progresses. Create your server by giving it a name and icon.
Once the server is created - it's yours. You can delete channels / change stuff around - rename stuff - 
whatever you'd like that makes sense for you.

I'll be implementing an automatic webhook creation process - but for now, you'll beed to make sure you add 
appropriate webhooks for the data you want to analyze.

For example - If you want to monitor BTC-USD and ETH-USD trades, you'll need to make sure you go to the BTC and ETH
channels, and make webhooks for them both.

You can find dictionaries containing the CRYPTO LOGO URLS and brief descriptions inside of the *discord_utils* folder.


Here's an example of what one of the Crypto feeds could potentially look like:

![Server Example](https://i.ibb.co/YZMqmzp/step3.png)

---

### Step 5a. Create your webhooks manually.

Right click any of the channels you want to incorporate data feeds into, and right click the channel.

Go to "Edit" --> Integrations --> "New Webhook" and name your webhook / add the Logo image if you wish.

![Create Webhook Example](https://i.ibb.co/LY86MdZ/step4.png)

![Webhook Example 2](https://i.ibb.co/KqkH8Q2/step5.png)

From there - you can utilize this repo to easily integrate real-time data into your server
to create a unique edge in financial markets.


### Step 5b. Create your webhooks automatically.

You can run the "channel_creation.py" channel to start creating webhooks automatically.
Just change the tickers in the provided list of SP500 tickers, or run it as is. It will create
channel names / webhooks and store them in a dictionary for later use.

![Example Auto-Webhooks](https://i.imgur.com/4N8TVlZ.gif)
...
An example implementation has been included in this repo that you can set-up.

1. Run the "channel_creation.py" file once you have made your server and retrieved your server's ID.

2. After the channel creation is complete - you'll have a CSV file located in files/discord/webhooks that will contain the
ticker names / webhook URLs for your server.

3. Run 'stock_market.py' to see the example feeds in action!


![Example Feeds](https://i.imgur.com/ZnBtMy6.gif)


#### More Complex Approach:


Here's an overview of Discord integration with both the PolygonSDK and the WebullSDK:

This code is an asynchronous Python script that uses the Polygon API, Webull API, and Discord Webhooks to collect financial data, analyze it, and send notifications to Discord channels based on various criteria.

Here's a detailed breakdown of the code:

#####  1. Import necessary libraries and modules.

##### 2. Initialize the `WebSocketClient` for Polygon API, which will listen to real-time trade data.

##### 3. Set up Discord Webhook URLs for various categories and criteria.

##### 4. Define a series of asynchronous functions that will be used to process and analyze the financial data:

   - `check_conditions()`: Check if the stock's MACD and RSI values meet a specified condition for different timeframes (day, hour, week).
   - `send_daybanana()`, `send_hourbanana()`, `send_weekbanana()`: Send notifications to Discord channels if the stock meets the MACD and RSI criteria for a specific timeframe.
   - `process_vol_analysis()`: Get analyst ratings for a stock and send notifications to Discord channels based on the rating.
   - `process_rsi()`: Get RSI values for a stock and send notifications to Discord channels if the stock is overbought or oversold.
   - `process_webull_data()`: Get various stock data from Webull API and send notifications to Discord channels based on criteria like 52-week highs and lows, earnings, etc.
   - `process_big_trades()`: Detect large trades and send notifications to Discord channels.
   - `handle_msg()`: Process each incoming WebSocket message from Polygon API and add the parsed data to a queue for further processing.
   - `process_conditions()`: Wrapper function to check conditions for all timeframes.
   - `process_data()`: Main worker function that processes data from the queue and calls the appropriate analysis functions.
--- 
#### 5. Define the `main()` function, which does the following:

   - Create a queue for storing financial data.
   - Connect to the Polygon API WebSocket and start listening to real-time trade data.
   - Create a specified number of worker tasks to process the data in the queue using the `process_data()` function.
   - Run the worker tasks concurrently using `asyncio.gather()`.

#### 6. Run the `main()` function using `asyncio.run()`.

The script listens to real-time trade data from the Polygon API, processes the data using various analysis functions, and sends notifications to Discord channels based on the conditions met.
...
import asyncio
import numpy as np
from sdks.polygon_sdk.stocks.technical_conditions import check_macd_condition,check_rsi_condition
...
from discord_utils.hooks.channel_webhooks import firesale,accumulate,neutzone, aboveavgvolume, belowavgvolume, fiftyhighh,fiftylowh, earningstoday, weekbanana,daybanana,hourbanana
...
async def check_conditions(symbol, timespan):
    ...
    async def get_conditions(timespan):
        ...
        macd_condition = await check_macd_condition(hist)
        rsi_condition = await check_rsi_condition(rsi)
...
analyst_tickers = set()
async def process_vol_analysis(symbol: str):
    global analyst_tickers
    # If the ticker has already been processed today, skip it
    if symbol in analyst_tickers:
        return

    # Add the ticker to the set of processed tickers
    analyst_tickers.add(symbol)
...
async def process_low_float(symbol):
    ...
    if totshares is not None and totshares < 10000000:
        ...
        if totshares < 10000000:
            ...
            low_floathook = AsyncDiscordWebhook(low_float, content="<@375862240601047070>")
            ...
            for k, v in result.items():
                ...
                if isinstance(v, float):
                    ...
                    if result['score'] is not None and result['score'] > 11:
                        ...
                        fin_scoregood = AsyncDiscordWebhook(good_financials)
                        description = '\n'.join(output_list)
                    ...
                    if result['score'] is not None and result['score'] < 6:
                        badfin_scorebad = AsyncDiscordWebhook(bad_financials)
                        baddescription = '\n'.join(output_list)
