...
from .top_options import BelongTicker, Values, Derivative
from typing import Dict,Union
from datetime import datetime, timedelta, timezone
from .financial_statement import FinancialStatement, CashFlow, BalanceSheet, Forecast
...
class AsyncWebullSDK:
    ...
    async def top_options_data(self, rank_type):
    ...
    async def analyze_capital_flow(self, ticker):
        """Gathers net flow for a ticker."""

        tickerid, _ = await self.fetch_ticker_id(ticker) # Fetch the tickerId for the given keyword
        ...
        if net_flow > 0:
            ...
            return f"{abs(net_flow)}"
        else:
            return "No net inflow or outflow"
    ...
    async def calculate_ratios(self, balance_sheet, fin_statement, cashflow, market_price):
        ...
        if fin_statement and balance_sheet:
            ...
            try:
                if cashflow[0].total_cash_dividends_paid is not None and fin_statement[0].net_income is not None and float(fin_statement[0].net_income) > 0:
                    dividend_payout_ratio = float(cashflow[0].total_cash_dividends_paid) / float(fin_statement[0].net_income)
                else:
                    dividend_payout_ratio = None
            except IndexError:
    ...
    async def get_webull_news(self, ticker, current_news_id: int = 0, page_size: int = 50, headers: dict = None) -> List[NewsItem]:
        async with aiohttp.ClientSession(headers=webull_headers) as session:
            ticker_id, _ = await self.fetch_ticker_id(ticker)
