...
class MainView(disnake.ui.View):
    ...
    @disnake.ui.button(
        ...
        label=f"Summarizer",
        row=0,
        custom_id="summarizeringyo",
    )
    async def summarize_button(
        self, button: disnake.ui.Button, interaction: disnake.ApplicationCommandInteraction
    ):
        await interaction.response.defer(with_message=True)
        await interaction.followup.send(
            "Please send the URL or file path you'd like to summarize.", ephemeral=False)
        ...
        try:
            user_message = await interaction.client.wait_for(
                "message", check=check, timeout=60
            )
        except asyncio.TimeoutError:
            await interaction.followup.send(
                "Time'sup! Please click the button again to retry.", ephemeral=True)
            return

        url_or_path = user_message.content
        ...
        # Split the summary into chunks of 2048 characters (the maximum length for embed descriptions)
        chunks = [summary[i: i + 2048] for i in range(0, len(summary), 2048)]
        data_s = { 
            'chunks': chunks,
            'summary': summary
        }
        df = pd.DataFrame(data_s)
        df.to_csv('files/summaries/government.csv')
        # Create a list of embeds for each chunk
        embeds = []
        for index, chunk in enumerate(chunks):
            embed = disnake.Embed(
                title="Summarized Text:",
                description=f"> {chunk}",
                color=disnake.Color.blurple()  # You can choose any color you like
            )
        ...
        await interaction.send(view=view,embed=embeds[0])
